...
import {
  Copy, ExternalLink, ImageOff, LoaderCircle, ShoppingBag, Package,
  AlertTriangle, Info,
} from "lucide-react";
...
export function ProductDetailsDialog({
  ...
  const name = data?.name ?? input.fallbackName ?? (isPass ? `Game pass ${input.productId}` : `Developer product ${input.productId}`);
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-left">
            {isPass ? <Package className="h-5 w-5 text-primary" /> : <ShoppingBag className="h-5 w-5 text-primary" />}
          ...
          <div className="min-w-0 flex-1">
            ...
            {fieldRow("For sale",
              data?.isForSale == null
                ...
                : data.isForSale ? "Yes" : "No")}
            {fieldRow("Created",
              createdAt ? new Date(createdAt).toLocaleString() : <span className="text-muted-foreground">Unknown</span>)}
            {fieldRow("Experience",
              data?.experienceName ?? input.experienceLabel ?? <span className="text-muted-foreground">—</span>)}
            {fieldRow("Place ID", input.placeId ?? <span className="text-muted-foreground">—</span>)}
            {fieldRow("Universe ID", input.universeId ?? <span className="text-muted-foreground">—</span>)}
            {fieldRow("Creator",
              data?.creatorName
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Button variant="outline" size="sm" onClick={() => void copyText(String(input.productId), "Product ID")}>
            <Copy /> Copy Product ID
          </Button>
          <Button variant="outline" size="sm" disabled={!input.universeId} onClick={() => input.universeId && void copyText(String(input.universeId), "Universe ID")}>
            <Copy /> Copy Universe ID
          </Button>
          <Button variant="outline" size="sm" disabled={!input.placeId} onClick={() => input.placeId && void copyText(String(input.placeId), "Place ID")}>
            <Copy /> Copy Place ID
          </Button>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          {creatorHubUrl && (
            <a href={creatorHubUrl} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center justify-center gap-2 rounded-md border border-white/10 px-3 py-2 text-sm hover:border-primary/50 hover:text-primary">
        ...
        {!isPass && (
          <div className="space-y-2 rounded-md border border-white/10 bg-background/40 p-3">
            ...
            <p className="text-xs text-muted-foreground">
              Individual developer products have no public roblox.com detail URL. Purchases must be prompted from
              inside the experience via <code>MarketplaceService:PromptProductPurchase</code>. This website cannot
              trigger a purchase directly.
            </p>
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => void copyText(luaSnippet, "LocalScript snippet")}>
