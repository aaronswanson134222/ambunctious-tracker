...
async function authorizeRequest(request: Request, client: AdminClient) {
  ...
  if (/^[a-f0-9]{64}$/i.test(token) && await booleanRpc(client, "verify_tracker_cron_secret", { candidate: token })) {
...
async function sendApiReleaseLog(client: AdminClient, errors: string[]) {
  const { data, error } = await client.from("tracker_api_releases")
    .select("version").eq("version", API_RELEASE.version).maybeSingle();
  if (error) {
...
async function recordStatus(client: AdminClient, results: ScanMetrics) {
  ...
  const { data: statusRow } = await db.from("tracker_discord_status")
    .select("message_id").eq("singleton", true).maybeSingle();
  const nextScan = Math.floor((Math.floor(Date.now() / 60_000) + 1) * 60);
  const checked = results.x_checked + results.products_checked + results.websites_checked + results.roblox_checked;
  const detected = results.x_new_posts + results.price_drops + results.website_updates + results.roblox_new_items;
  const payload = {
    embeds: [{
      ...
      fields: [
        { name: "Total scans", value: `**${Number(count ?? 0).toLocaleString("en-GB")}**`, inline: true },
        ...
        { name: "Health", value: results.errors.length ? `${results.errors.length} warning(s)` : "All checks healthy", inline: true },
      ],
  ...
  await db.from("tracker_discord_status").upsert(
...
async function runChecks() {
  ...
  const results: ScanMetrics = {
    x_checked: 0, x_new_posts: 0, products_checked: 0, price_drops: 0,
    websites_checked: 0, website_updates: 0, roblox_checked: 0,
    roblox_new_items: 0, discord_sent: 0, release_notifications: 0,
    hourly_summaries: 0, errors: [],
  };
  ...
  const { data: xs, error: xErr } = await db.from("tracked_x_accounts").select("id,handle,last_post_url");
  ...
  for (const row of xs ?? []) {
    ...
    try {
      ...
      if (isNew && postUrl) {
        const reservation = await reserveNotification(supabaseAdmin, "x_post", row.id, current.toString());
        if (reservation) {
          try {
      ...
      await db.from("tracked_x_accounts").update({
      ...
      await db.from("tracked_x_accounts").update({ last_checked_at: new Date().toISOString(), last_error: message.slice(0, 500) }).eq("id", row.id);
    }
  }

  const { data: trackedProducts, error: productsError } = await db.from("tracked_products")
  ...
  for (const row of trackedProducts ?? []) {
    ...
    try {
      ...
      if (row.last_price != null && price < Number(row.last_price)) {
        ...
        const reservation = await reserveNotification(supabaseAdmin, "price_drop", row.id, `${effectiveCurrency}:${price}`);
        if (reservation) {
          try {
            await sendDiscord({ embeds: [{
              title: `🔻 Price drop: ${row.label}`, url: row.url,
              ...
              color: 0x22c55e, timestamp: new Date().toISOString(),
            }] });
            await reservation.markSent();
            results.price_drops++;
            results.discord_sent++;
      ...
      await db.from("price_history").insert({ product_id: row.id, price, currency: effectiveCurrency, price_gbp: priceGbp });
      await db.from("tracked_products").update({ last_price: price, last_price_gbp: priceGbp, currency: effectiveCurrency, last_checked_at: new Date().toISOString(), last_error: null }).eq("id", row.id);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      results.errors.push(`product/${row.label}: ${message}`);
      await db.from("tracked_products").update({ last_checked_at: new Date().toISOString(), last_error: message.slice(0, 500) }).eq("id", row.id);
    }
  }

  const { data: openCloudKey } = await db.rpc("get_roblox_open_cloud_key");
  const { data: entities, error: entityError } = await db.from("tracked_roblox_entities")
  ...
  for (const row of entities ?? []) {
    ...
    try {
      const scanTypes = (Array.isArray(row.scan_types) ? row.scan_types : ["catalog", "experience"])
        .filter((value: string) => ["catalog", "experience", "game_pass", "developer_product"].includes(value));
      const baselined = new Set(Array.isArray(row.baselined_scan_types) ? row.baselined_scan_types : []);
      const known = new Set(Array.isArray(row.known_item_keys) ? row.known_item_keys : []);
      const creations = await checkRobloxCreations(row.entity_type, Number(row.entity_id), scanTypes, Number(row.lookback_days), typeof openCloudKey === "string" ? openCloudKey : null);
      const fresh = creations.filter((item) => baselined.has(item.kind) && !known.has(item.key)).slice(0, 10);
      for (const item of fresh) {
        const reservation = await reserveNotification(supabaseAdmin, "roblox_creation", row.id, item.key);
        ...
        try {
          const preview = await enrichRobloxPreview({
            ...
            universeId: ["game_pass", "developer_product"].includes(item.kind) ? universeFromKey(item.key) : item.kind === "experience" ? item.id : null,
          });
          await sendDiscord({ embeds: [buildRobloxPreviewEmbed(preview, `ROBLOX ${String(row.entity_type).toUpperCase()} // ${row.label}`)] });
          await reservation.markSent();
      ...
      await db.from("tracked_roblox_entities").update({
        known_item_keys: [...new Set([...creations.map((item) => item.key), ...known])].slice(0, 500),
      ...
      await db.from("tracked_roblox_entities").update({ last_checked_at: new Date().toISOString(), last_error: message.slice(0, 500) }).eq("id", row.id);
    }
  }

  const { data: experienceRows, error: experienceError } = await db.from("tracked_roblox_experiences")
  ...
  for (const row of experienceRows ?? []) {
    results.roblox_checked++;
    try {
      const rawProducts = await checkRobloxExperienceProducts(Number(row.universe_id), Number(row.place_id), Number(row.lookback_days), typeof openCloudKey === "string" ? openCloudKey : null);
      const products = await Promise.all(rawProducts.map((item) => enrichRobloxPreview({
        id: item.id,
        kind: item.kind,
        name: item.name,
        url: item.url,
      ...
      const fresh = products.filter((item) => !known.has(item.key) && (item.kind !== "developer_product" || hasDevBaseline)).slice(0, 20);
      for (const item of fresh) {
        const reservation = await reserveNotification(supabaseAdmin, "roblox_experience_product", row.id, item.key);
        if (!reservation) continue;
        try {
          const preview = await enrichRobloxPreview({
            id: item.id, kind: item.kind, name: item.name, url: item.url,
            createdAt: item.createdAt, universeId: Number(row.universe_id),
            placeId: Number(row.place_id), experienceName: row.label,
          });
          await sendDiscord({ embeds: [buildRobloxPreviewEmbed(preview, `ROBLOX EXPERIENCE // ${row.label}`)] });
          await reservation.markSent();
          results.roblox_new_items++;
          results.discord_sent++;
        } catch (error) { await reservation.release(); throw error; }
      }
      await db.from("tracked_roblox_experiences").update({
        known_item_keys: [...new Set([...products.map((item) => item.key), ...known])].slice(0, 1000),
        items: products,
        last_checked_at: new Date().toISOString(), last_error: null, updated_at: new Date().toISOString(),
      }).eq("id", row.id);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      results.errors.push(`roblox experience/${row.label}: ${message}`);
      await db.from("tracked_roblox_experiences").update({ last_checked_at: new Date().toISOString(), last_error: message.slice(0, 500), updated_at: new Date().toISOString() }).eq("id", row.id);
    }
  }

  const { data: websites, error: websitesError } = await db.from("tracked_websites").select("id,label,url,last_item_url");
  ...
  for (const row of websites ?? []) {
    ...
    try {
      ...
      if (row.last_item_url !== null && update.itemUrl !== row.last_item_url) {
        const reservation = await reserveNotification(supabaseAdmin, "website_update", row.id, update.itemUrl);
        if (reservation) {
          try {
            await sendDiscord({ embeds: [{
              author: { name: "BIG Games // Website update", icon_url: "https://www.biggames.io/favicon.ico", url: "https://www.biggames.io/post" },
              title: `New BIG Games update: ${update.title}`, url: update.itemUrl,
              ...
              color: 0xf4c542, timestamp: new Date().toISOString(),
            }] });
            await reservation.markSent();
      ...
      await db.from("tracked_websites").update({ last_item_url: update.itemUrl, last_item_title: update.title, last_checked_at: new Date().toISOString(), last_error: null }).eq("id", row.id);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      results.errors.push(`website/${row.label}: ${message}`);
      await db.from("tracked_websites").update({ last_checked_at: new Date().toISOString(), last_error: message.slice(0, 500) }).eq("id", row.id);
    }
  }
  ...
  try {
    ...
    results.errors.push(`permanent Discord status: ${error instanceof Error ? error.message : String(error)}`);
  }
  return results;
}
  ...
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        if (!acquired) return json({ error: "A tracker scan is already running or ran moments ago." }, 409);
        try {
          ...
          try { await booleanRpc(supabaseAdmin, "release_tracker_run_lock", {}); }
          catch (error) { console.error("Could not release tracker run lock", error); }
        }
      },
    },
  },
