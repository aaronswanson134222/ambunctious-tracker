...
function EmbedTester() {
  ...
  function useLatestSaved() {
    ...
    if (!latest) {
      toast.info(`No saved ${kind === "game_pass" ? "game passes" : "developer products"} were found.`);
      return;
    }
  ...
  return (
    <Card className="tracker-card space-y-5 p-5 sm:p-6">
      ...
      {loading ? (
        <div className="flex items-center gap-2 text-sm text-muted-foreground"><LoaderCircle className="animate-spin" size={18} /> Loading tracked experiences…</div>
      ) : !experiences.length ? (
        <p className="text-sm text-muted-foreground">Add a Roblox experience on the tracker dashboard before using the embed tester.</p>
      ) : (
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="space-y-2 text-sm">
              <span className="panel-label">EXPERIENCE</span>
              <select className="command-input h-11 w-full rounded-xl px-3" value={experienceId} onChange={(event) => { setExperienceId(event.target.value); setResult(null); }}>
                {experiences.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
              </select>
            </label>
            <label className="space-y-2 text-sm">
              <span className="panel-label">PRODUCT TYPE</span>
              <select className="command-input h-11 w-full rounded-xl px-3" value={kind} onChange={(event) => { setKind(event.target.value as typeof kind); setProductId(""); setFallbackName(""); setResult(null); }}>
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
            <Input value={productId} onChange={(event) => setProductId(event.target.value.replace(/\D/g, ""))} placeholder="Product ID" className="h-11 rounded-xl" />
            <Input value={fallbackName} onChange={(event) => setFallbackName(event.target.value)} placeholder="Fallback product name" className="h-11 rounded-xl" />
            <Button variant="outline" onClick={useLatestSaved} className="h-11 rounded-xl"><Gamepad2 size={17} /> Use latest saved</Button>
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            <Input value={customTitle} onChange={(event) => setCustomTitle(event.target.value)} maxLength={256} placeholder="Custom embed title (optional)" className="h-11 rounded-xl" />
            <Textarea value={customMessage} onChange={(event) => setCustomMessage(event.target.value)} maxLength={3500} placeholder="Custom message (optional)" className="min-h-11 rounded-xl" />
          </div>
        ...
        <div className="content-panel space-y-4 p-4">
          <div className="flex items-start gap-4">
            {result.details.thumbnailUrl
              ? <img src={result.details.thumbnailUrl} alt="" className="h-24 w-24 rounded-xl object-cover" />
              : <div className="tracker-avatar"><Gamepad2 /></div>}
            <div className="min-w-0 flex-1">
              <p className="flex items-center gap-2 text-sm text-emerald-400"><CheckCircle2 size={16} /> {result.sent ? "Test sent" : "Preview ready"}</p>
              <h3 className="mt-1 truncate text-xl font-semibold">{result.details.name}</h3>
              <p className="mt-1 text-sm text-muted-foreground">
                {result.details.priceInRobux == null ? "Price unavailable" : `${result.details.priceInRobux.toLocaleString("en-GB")} Robux`}
                {result.details.experienceName ? ` · ${result.details.experienceName}` : ""}
              </p>
              {result.details.creatorName && <p className="mt-1 text-xs text-muted-foreground">Creator: {result.details.creatorName}</p>}
              {result.details.description && <p className="mt-3 line-clamp-3 text-sm text-muted-foreground">{result.details.description}</p>}
            </div>
          </div>
          {result.details.warnings.length > 0 && <p className="text-xs text-amber-300">{result.details.warnings.join(" • ")}</p>}
        </div>
      )}
    </Card>
  );
    ...
    <main className="min-h-screen bg-background px-4 py-8 text-foreground sm:px-6">
      <div className="mx-auto max-w-5xl space-y-6">
        <Card className="tracker-card p-6 sm:p-8">
          <p className="eyebrow"><span /> AMBUNCTIOUS CONTROL CENTRE</p>
          <div className="mt-3 flex items-start gap-4">
            <div className="tracker-avatar"><ShieldCheck /></div>
            <div><h1 className="text-3xl font-semibold">Main menu</h1><p className="mt-2 text-sm text-muted-foreground">Everything for monitoring, Discord alerts, connection health and puzzle solving in one place.</p></div>
          </div>
        </Card>
        <section className="grid gap-4 sm:grid-cols-2">
          {items.map((item) => {
            const Icon = item.icon;
            return <Link key={item.to} to={item.to} className="block"><Card className="tracker-card h-full p-5 transition hover:-translate-y-0.5 hover:border-primary/50"><div className="flex items-start gap-4"><div className="tracker-avatar"><Icon /></div><div><h2 className="text-lg font-semibold">{item.title}</h2><p className="mt-2 text-sm text-muted-foreground">{item.copy}</p></div></div></Card></Link>;
          })}
        </section>
        <EmbedTester />
