  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <div className="mt-6"><Link to="/menu" className="inline-flex items-center justify-center rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition-colors hover:bg-primary/90">Open main menu</Link></div>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="max-w-md text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">Something went wrong on our end. You can try refreshing or open the main menu.</p>
        <div className="mt-6 flex flex-wrap justify-center gap-2">
          ...
          <a href="/menu" className="inline-flex items-center justify-center rounded-md border border-input bg-background px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-accent">Main menu</a>
        </div>
      </div>
    </div>
  );
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Monitor X accounts, Roblox activity and Eldorado listing prices with automatic one-minute scans and Discord alerts." },
      { property: "og:title", content: "Ambunctious Tracker — X posts & price alerts" },
      { property: "og:description", content: "Monitor X accounts, Roblox activity and Eldorado listing prices with automatic one-minute scans and Discord alerts." },
...
function OneMinuteScanUiSync() {
  useEffect(() => {
    const update = () => {
      ...
      document.querySelectorAll<HTMLElement>(".countdown-node strong, .top-countdown strong").forEach((element) => { element.textContent = value; });
      document.querySelectorAll<HTMLElement>(".countdown-node i b, .top-countdown i b").forEach((element) => { element.style.width = progress; });
      document.querySelectorAll<HTMLElement>("p, span, small").forEach((element) => {
        if (element.children.length) return;
        const text = element.textContent ?? "";
        const next = text.replace(/Five-minute scans\.?/gi, "One-minute scans.").replace(/Checked every 5 minutes/gi, "Checked every 60 seconds").replace(/SECURE FIVE-MINUTE CYCLE/gi, "SECURE 60-SECOND CYCLE").replace(/05 MIN/g, "60 SEC");
